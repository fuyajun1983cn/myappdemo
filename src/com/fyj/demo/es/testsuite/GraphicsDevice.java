...
public class GraphicsDevice {
	...
	public LinkedList<Integer> findValidEGLConfig(TestDescriptor testDescriptor) {

		LinkedList<Integer> configIdWithAlphaList = new LinkedList<Integer>();
		LinkedList<Integer> configIdWithoutAlphaList = new LinkedList<Integer>();
		...
		int targetRBit = testDescriptor.getColorBpp() == 24 ? 8 : 5;
		int targetGBit = testDescriptor.getColorBpp() == 24 ? 8 : 6;
		int targetBBit = testDescriptor.getColorBpp() == 24 ? 8 : 5;
		...
		for (int i = 0; i < numConfigs[0]; i++) {
			...
			if (supported[0] < testDescriptor.getDepthBpp())
				continue;

			egl10.eglGetConfigAttrib(eglDisplay, configs[i],
					EGL10.EGL_SAMPLE_BUFFERS, supported);
			if (testDescriptor.getFsaa() > 0 && supported[0] == 0
					|| testDescriptor.getFsaa() == 0 && supported[0] == 1)
				continue;

			egl10.eglGetConfigAttrib(eglDisplay, configs[i], EGL10.EGL_SAMPLES,
					supported);
			if (testDescriptor.getFsaa() != 0) {
				if (supported[0] != testDescriptor.getFsaa())
					continue;
			}
			...
			if (alpha[0] == 0)
				configIdWithoutAlphaList.add(id[0]);
			else
				configIdWithAlphaList.add(id[0]);

		}

		LinkedList<Integer> configIdList = new LinkedList<Integer>(
				configIdWithoutAlphaList);
		configIdList.addAll(configIdWithAlphaList);
	...
	public boolean createGLSurface(EGLConfig eglConfig,
		...
		if (eglSurface == EGL10.EGL_NO_SURFACE) {
			TLogger.Error("NOSURFACE");
		}
		if (eglDisplay == EGL10.EGL_NO_DISPLAY) {
			TLogger.Error("NODISPLAY");
		}

		if (eglContext == EGL10.EGL_NO_CONTEXT) {
			TLogger.Error("NOCONTEXT");
		}
